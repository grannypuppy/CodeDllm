...
- Code tokens (AST-mapped): depth-based selection (deeper first).
...
def prepare_inputs_and_labels_for_text(
    ...
    - Code tokens: depth-based masking (deeper first).
    ...
    for i, (inp, tgt) in enumerate(zip(input_list, target_list)):
        ...
        # Initialize depth labels with -1 (non-code / ignored)
    ...
    for b in range(B):
        ...
        for ratio in ratios:
            ...
            # Code: depth-based selection
            code_mask_global = get_depth_based_code_masking(
                ast_node_info=ast_info,
                code_token_indices=code_token_indices,
                num_to_mask_code=num_to_mask_code,
                response_start_pos=current_start_pos,
            )
    ...
    if noisy_list:
        ...
        raise ValueError(
            "No valid samples for depth-based masking. Check data and config."
        )
...
def main():
    ...
    target_list = []
    step_map_list = []
    # Initialize batch_depth_labels list
    batch_depth_labels = [] 
