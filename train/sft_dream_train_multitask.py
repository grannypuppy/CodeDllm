...
def main():
    ...
    if resume_from_checkpoint:
        ...
        model = DreamMultitaskModel.from_pretrained(load_path, config=model_config, torch_dtype=torch.bfloat16)
    ...
    def forward_process(input_ids, labels, p_mask_lm, depth_labels, start_pos):
        ...
        # --- Total Loss ---
        rank_loss_weight = config.training.get("rank_loss_weight", 0.01)
        loss_total = loss_lm + (rank_loss_weight * loss_rank)
    ...
    for epoch in range(first_epoch, num_train_epochs):
        ...
        for step, batch in enumerate(progress_bar, start=1):
            ...
            # --- Depth label extraction (label source remains `depth_labels`) ---
