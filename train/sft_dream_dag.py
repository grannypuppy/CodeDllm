...
class TrainDataset(Dataset):
    def __init__(self, inputs, labels, pmasks, weights=None):
        self.inputs = inputs
        self.labels = labels
        self.pmasks = pmasks
...
def main():
    ...
    def prepare_inputs_and_labels_for_text(
        input_list, target_list, step_map_list, target_weights_list=None, eps=1e-3, mask_id=mask_id
    ):
        # Templates
        ...
        seg_prompting = SegmentedPrompting(tokenizer, max_prompt_len, max_gen_length)
        seg_res = seg_prompting.get_batch_data(
             input_list, target_list, target_weights_list, 
             src_code_prompts_template, tgt_code_suffix
        )
        
        if seg_res is None:
            raise ValueError("No valid data found in the batch")
             
        input_ids_lm, labels_lm, batch_weights, start_pos, keep_indices, batch_response_lens = seg_res
        ...
        logger.info(f"start_pos: {start_pos}; input_tokens_at_start_pos: {tokenizer.decode(input_ids_lm[2][start_pos], skip_special_tokens=True)}")
        # Step Map filtering
        ...
        final_weights_out = None

        if config.training.method == "semi-ar":

            noisy_list, label_list, pmask_list, weight_list = [], [], [], []
            ...
            for b in range(B):
                ...
                resp_len = len(order)
                order_full[start_pos : start_pos + resp_len] = order
                ...
                for i in range(0, len(uniq_steps)):
                    ...
                    if batch_weights is not None:
                        weight_list.append(batch_weights[b])
            ...
            if noisy_list:
                ...
                if batch_weights is not None:
                    final_weights_out = torch.stack(weight_list)
            else:
                raise ValueError("No valid data found in the batch")
            ...
            if batch_weights is not None:
                final_weights_out = batch_weights
        ...
        if config.training.method != "ar":
             ...
             if final_weights_out is not None:
                 final_weights_out = final_weights_out[valid_rows]
            
        return noisy_batch, labels_lm, p_mask, start_pos, drop_num, final_weights_out
        

    def simple_collate(batch):
        inp, lbl, msk, wgt = zip(*batch)
        res = {
    ...
    input_ids, labels, p_mask_lm, start_pos, drop_num, final_weights = prepare_inputs_and_labels_for_text(
        input_list, target_list, step_map_list, target_weights_list
    )

    
    dataset_lm = TrainDataset(input_ids, labels, p_mask_lm, final_weights)
    ...
    for epoch in range(first_epoch, num_train_epochs):
        ...
        for step, batch in enumerate(progress_bar, start=1):
            ...
            if step <= 1 and accelerator.is_local_main_process:
