...
def run(cfg):
    ...
    def forward_process(input_ids, labels, p_mask_lm, adv, logp_old_tok, rank_mask, rv_old_dense):
        ...
        # ---- Rank head GRPO (PPO-clip, Gaussian policy for continuous action) ----
        #
        # Interpretation:
        #   rank_value (scalar) = the ACTION produced by rank_head at each code position.
        #   We model rank_head as a Gaussian policy:
        #       p(v | context) = N(rank_head_output, sigma_rank^2)
        #   where sigma_rank is a fixed hyperparameter.
        #
        # During rollout (deterministic):
        #   y_old = mu_old = rank_head_rollout_output   (= rv_old, saved in rank_step_records)
        #
        # log p_old(y_old) = -0.5 * ((y_old - mu_old) / sigma)^2 + const = 0 + const
        #                    (mode of N(mu_old, sigma^2), so zero squared-error)
        #
        # log p_new(y_old) = -0.5 * ((y_old - mu_new) / sigma)^2 + const
        #                  = -0.5 * ((rv_old - rv_new) / sigma)^2 + const
        #
        # log ratio = log p_new - log p_old
        #           = -0.5 / sigma^2 * (rv_old - rv_new)^2
        #
        # ratio = exp(-0.5 / sigma^2 * (rv_old - rv_new)^2)
        #       ∈ (0, 1]   (= 1 when model unchanged, < 1 as model deviates)
        #
        # Gradient signal:
        #   adv > 0: pushes rv_new → rv_old (reinforce rollout rank behavior)
        #   adv < 0: pushes rv_new away from rv_old (penalize rollout rank behavior)
        #
        # Vectorized like LM: dense mask [B, T] + PPO surrogate, then mean over supervised
        # positions per sample, then mean over samples that have any rank supervision.
        sigma_rank = float(cfg.training.get("rank_sigma", 0.1))
        valid = rank_mask  # [B, T]
        rv_old = rv_old_dense.to(dtype=rank_values.dtype)
        log_ratio_rank = -0.5 / (sigma_rank ** 2) * (rv_old - rank_values).pow(2)
        ratio_rank = torch.exp(log_ratio_rank)
        ratio_clipped = torch.clamp(ratio_rank, 1 - cfg.training.eps, 1 + cfg.training.eps)
        adv_expand = adv_t.unsqueeze(1)  # [B, 1]
        surr = torch.min(ratio_rank * adv_expand, ratio_clipped * adv_expand)  # [B, T]
        valid_f = valid.to(dtype=surr.dtype)
        surr_sum = (surr * valid_f).sum(dim=1)
        valid_count = valid_f.sum(dim=1)
        mean_per_sample = torch.where(
            valid_count > 0,
            surr_sum / valid_count.clamp(min=1.0),
            # Use surr_sum * 0.0 instead of torch.zeros_like: this keeps rank_values
            # in the autograd graph even when no rank supervision is present in this
            # rank's batch slice.  With DeepSpeed ZeRO-3 + split_batches=True some
            # ranks may have no rank data; a detached constant (torch.zeros_like)
            # would sever the gradient path through rank_head, causing those ranks to
            # skip the reduce-scatter hook → collective-op mismatch → NCCL timeout.
            # surr_sum * 0.0 produces the same zero value but fires all ZeRO-3
            # backward hooks consistently across every rank.
            surr_sum * 0.0,
        )
        has_rank = valid_count > 0
        # Always vectorized – no Python branch on has_rank so all ranks follow the
        # same autograd graph shape regardless of whether their batch slice contains
        # rank supervision data.
        num_with_rank = has_rank.float().sum().clamp(min=1.0)
        rank_policy_loss = -(mean_per_sample * has_rank.float()).sum() / num_with_rank
