...
# 全局序列：长度 m，指定每次 mask 时「要 mask 的比例」（占可 mask token 数的比例）
# 如 0.05 表示本次 mask 的 token 数 = int(0.05 * num_maskable_tokens)。不再按概率随机，直接按比例取
# 可自行修改列表，长度即为 m（m 次比例 mask，再加上脚本内固定的一次全 mask，共 m+1 次）
...
def prepare_inputs_and_labels_for_text(
    ...
    if config.training.method == "semi-ar":
        ratios = config.training.get("random_masking_ratios", None)
        if ratios is None:
            ratios = RANDOM_MASKING_RATIOS
        block_size = config.training.get("block_size", 32)
        ...
        for b in range(B):
            ...
            # 直接用 batch_step_map[b] 做 collapse，无需中间变量 order_list
            order = torch.as_tensor(
                collapse_k_unique(list(batch_step_map[b]), block_size),
                device=device,
            )
            ...
            # 与 random_masking 一致：maskable 区域含 padding，padding 归入最后一个 block
            if resp_len > 0 and current_start_pos + resp_len < L_after:
                order_full[current_start_pos + resp_len : L_after] = order[-1]
            ...
            for i in range(0, len(uniq_steps)):
                ...
                num_maskable = block_mask.sum().item()
                # 复用 random_masking 的按比例选位置逻辑
                for ratio in ratios:
                    num_to_mask = int(ratio * num_maskable)
                    if num_to_mask <= 0:
                        continue
                    perm = torch.randperm(num_maskable, device=device)
                    block_indices = block_mask.nonzero(as_tuple=True)[0]
                    selected_in_block = block_indices[perm[:num_to_mask]]
                    rand_mask = torch.zeros(L_after, dtype=torch.bool, device=device)
                    rand_mask[selected_in_block] = True

                    noisy_ids = base_ids.clone()
                    mask_pos = (order_full > uniq_steps[i]) | rand_mask
                    noisy_ids[mask_pos] = mask_id
                    pmask_this = rand_mask
            ...
            # 与 random_masking 一致：每个样本加一条全 mask（整段 [current_start_pos, L_after)，含 padding）
        ...
        # 使用全局比例列表：每次 mask 的数量 = int(ratio * num_maskable_tokens)，不再随机概率
        ratios = config.training.get("random_masking_ratios", None)
        if ratios is None:
            ratios = RANDOM_MASKING_RATIOS
        m = len(ratios)
        ...
        for b in range(B):
            ...
            for ratio in ratios:
                ...
                # 在 [current_start_pos, L_after) 中随机选 num_to_mask 个位置
                perm = torch.randperm(num_maskable, device=device)
                selected = perm[:num_to_mask] + current_start_pos
                rand_mask = torch.zeros(L_after, dtype=torch.bool, device=device)
                rand_mask[selected] = True
            ...
            # Add full mask sample (mask all non-prompt tokens)，即第 m+1 次
            full_mask = torch.ones(L_after, dtype=torch.bool, device=device)
            full_mask[:current_start_pos] = False
