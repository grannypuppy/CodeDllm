...
def main():
    ...
    @torch.no_grad()
    def diffusion_step_update(x, logits, i, steps, timesteps, mask_token_id, alg, alg_temp, temperature, top_p, top_k, threshold, device, state=None, start_pos_1d=None):
        ...
        start_pos_1d: (B,) generation start per sample; only non-mask positions in [start_pos, L) are updated with x0_full (prompt is never overwritten)."""
