...
def main():
    ...
    # Dream multitask RL (margin version): always AST rollout under models/dream_multitask + dual-head train script.
    ...
    if use_dream_multitask:
        ...
        train_script = root / "train" / "rl_dream_train_multitask_margin.py"
        if not rollout_script.exists():
            raise FileNotFoundError(f"multitask rollout script not found: {rollout_script}")
        if not train_script.exists():
            raise FileNotFoundError(f"multitask RL margin train script not found: {train_script}")
    else:
    ...
    # When eval_every > 0, log a baseline on the starting (SFT) checkpoint as wandb round 0
    # before the first train rollout. Skip when resuming mid-run (start_round > 1).
    # baseline_model_path = str(Path(str(cfg.model.pretrained_model)).resolve())
    # if eval_every > 0 and start_round == 1:
    #     print(
    #         f"\n[RL] ===== Baseline evaluation (experiment.current_round=0, base model, before round 1 rollout) =====\n"
    #         f"[RL] model_path={baseline_model_path}\n"
    #         f"[RL] rollout_script={rollout_script_name}"
    #     )
    #     _run_rollout_stage(
    #         script=rollout_script,
    ...
    #             "experiment.function": "evaluation",
    #             "model.pretrained_model": baseline_model_path,
    #         },
    #     )
    #     _run_single_stage(
    #         script=reward_script,
    ...
    #         },
    #     )
    #     print("[RL] Baseline evaluation (round 0) done; continuing with RL rounds.\n")
