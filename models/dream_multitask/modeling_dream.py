...
class MaskedLMOutputWithPastKeyValues(MaskedLMOutput):
    def __init__(
        ...
        rank_values: Optional[torch.Tensor] = None,
        **kwargs,
    ):
...
class DreamPreTrainedModel(PreTrainedModel):
    ...
    def from_pretrained(
        ...
        **kwargs,
        ...
        # HF returns (model, loading_info) when output_loading_info=True.
        if isinstance(loaded, tuple):
            _model, loading_info = loaded
        else:
            _model, loading_info = loaded, None
...
class DreamModel(DreamGenerationMixin, DreamPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.rank_head = nn.Linear(config.hidden_size, config.num_rank_labels, bias=False)
    ...
    def forward(
        ...
        **loss_kwargs,
        ...
        rank_values = self.rank_head(hidden_states)
        ...
        if not return_dict:
            output = (logits, rank_values) + outputs[1:]
