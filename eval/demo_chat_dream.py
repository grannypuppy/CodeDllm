...
# Configure logging so AST detail logs from generation_utils_ast_detail_log are visible
...
# Config: aligned with gen_dream_eval.sh (244-256) and gen_dream_eval_ast.py
...
# Patch with AST generation + detail logging (same logic as gen_dream_eval_ast.py, using detail_log for [AST] logs)
from models.dream.generation_utils_ast_detail_log import DreamGenerationMixin
...
# Aligned with gen_dream_eval.py: apply_chat_template + tgt_code_response_prefix
USE_RSP_PREFIX = True
...
# Generate response: kwargs aligned with gen_dream_eval_ast.py / gen_dream_eval.sh (244-256)
gen_kwargs = {
...
# Decode same as gen_dream_eval_ast.py: skip prompt, then decode with skip_special_tokens
