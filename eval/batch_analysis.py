...
Speedup is calculated using avg_instruction_count when available, falling back to avg_run_time.
...
def get_metrics(result_entry):
    ...
    Returns: (correctness, runtime, compiled, instruction_count)
    ...
    runtime = overview.get("avg_run_time") or overview.get("runtime")
    runtime = float(runtime) if runtime is not None else float("inf")

    instruction_count = overview.get("avg_instruction_count")  # int or None

    return correctness, runtime, compiled, instruction_count
...
def analyze_run(baseline_data, target_data, nrows2analyze):
    ...
    Speedup is computed from avg_instruction_count when both baseline and target
    have it; otherwise falls back to avg_run_time.
    ...
    for i, (base_entry, target_entry) in enumerate(zip(baseline_data, target_data)):
        ...
        if base_eval_results:
            base_correct, base_runtime, _, base_ic = get_metrics(base_eval_results[0])

        # Target samples
        row_samples = []
        for j, res in enumerate(target_entry.get("eval_results", [])):
            correct, runtime, compiled, ic = get_metrics(res)

            speedup = 0.0
            if correct and base_correct:
                # Prefer instruction_count; fall back to runtime
                if ic is not None and base_ic is not None and base_ic > 0 and ic > 0:
                    speedup = base_ic / ic
                elif base_runtime > 0 and runtime > 0 and runtime != float("inf"):
